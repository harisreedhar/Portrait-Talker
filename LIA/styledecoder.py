...
class ModulatedConv2d(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, style_dim, demodulate=True, upsample=False,
                 downsample=False, blur_kernel=np.array([1, 3, 3, 1]), ):
        super().__init__()
...
class ToFlow(nn.Module):
    ...
    def forward(self, input, style, feat, skip=None):
        ...
        xs = torch.tensor(xs, requires_grad=False).float().unsqueeze(0).repeat(input.size(0), 1, 1, 1).to(device)
...
def custom_diag_embed(diagonal):
    size = diagonal.size(-1)
    eye = torch.eye(size, device=diagonal.device, dtype=diagonal.dtype)
    return diagonal.unsqueeze(-1) * eye
#------------------ custom OP ------------------#
